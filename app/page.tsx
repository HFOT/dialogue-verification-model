...
import { CRITERIA, CRITERION_EFFECT, CRITERION_TINY, STATE_LABEL, STATE_MARK, criteriaStateOf, shiftsFor, tallyOf } from './data/criteria';
...
export default function Home(){
  ...
  const pointDetail=useMemo(()=>{
    ...
    const criterion=CRITERIA.find(item=>item.id===shift.id);
    return{name:criterion?.label??'',tiny:CRITERION_TINY[shift.id],effect:CRITERION_EFFECT[shift.id][shift.state],state:shift.state,count:pointMetrics[observedMessage.id]?.n??1};
