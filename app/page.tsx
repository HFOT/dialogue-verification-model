...
import { CRITERIA, STATE_LABEL, STATE_MARK, criteriaStateOf } from './data/criteria';
...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    const postTimer=window.setTimeout(()=>{setTyping('');setVisible(previous=>previous.some(message=>message.id===next.id)?previous:[...previous,next].slice(-80));setCursor(value=>value+1)},speed);
  ...
  },[cursor,paused,awaitingChoice,sequence,speed]);
  ...
  const changeMode=(next:Mode)=>{
    ...
    if(commonSeenRef.current&&next!=='verify'){setVisible(base);setCursor(base.length)}
  ...
  const showBranch=mode==='ambiguous'&&atEnd;
  const showTail=(mode==='pressure'||mode==='open')&&atEnd;
  ...
  return <main className="site-shell"><nav className="topbar" aria-label="Dialogue Verification Model navigation"><a className="brand" href="#top" aria-label="Dialogue Verification Model home"><span className="brand-mark"/>DIALOGUE VERIFICATION MODEL</a><span className="nav-statement">Evidence-informed conversation study</span><span className="nav-status"><i/> SYNTHETIC MODEL</span></nav>
    ...
    <section className="experience" aria-label="Chat pattern simulator">
      ...
      <div className="chat-window phone-chat"><header className="chat-head"><div className="room-icon">O</div><div><b>Open Network Talk</b><span>架空の招待制オープンチャット · 43人</span></div><div className="room-lock">🔒 架空設定 / 運用グループ確認</div></header><div className="pinned"><b>このシミュレーションについて</b><span>会話・人物・資料はすべて架空です。参加者が自分で検証できる情報環境を考えます。</span></div><div className="message-feed phone-feed" ref={feedRef} onScroll={(event)=>{const node=event.currentTarget;followRef.current=node.scrollHeight-node.scrollTop-node.clientHeight<56}} aria-live="polite">{visible.map((message,index)=><article className={`message ${message.role??''}`} key={`${message.id}-${index}`}><div className="avatar">{message.initial}</div><div className="message-body"><div className="message-meta"><b>{message.author}</b><span>21:{String(4+index).padStart(2,'0')}</span>{message.role==='critical'&&<em>検証メモ</em>}</div><div className="bubble">{message.text}{message.point&&<button className="point" aria-label={`${message.author}の発話を観察パネルに表示`} aria-pressed={activePoint===message.point.note} onClick={()=>setActivePoint(message.point?.note??null)}>POINT</button>}</div>{message.preview&&<a className="link-preview" href={message.preview.url} target="_blank" rel="noreferrer"><b>{message.preview.title}</b><span>{message.preview.meta}</span><small>{message.preview.url}</small></a>}<div className="reactions">{message.reactions.length?message.reactions.map(reaction=><span key={reaction}>{reaction}</span>):<span className="seen">既読 31 · 反応なし</span>}{message.silentCore&&<span className="seen">圧力高め型BOT · 既読 · 反応なし</span>}</div></div></article>)}{showBranch&&<div className="branch-marker"><b>分岐点 · 中間型はここまで</b><span>{BRANCH_NOTE}</span><div className="branch-actions"><button type="button" onClick={()=>changeMode('pressure')}>圧力側の続きを見る</button><button type="button" onClick={()=>changeMode('open')}>非圧力側の続きを見る</button></div></div>}{showTail&&<div className="branch-marker"><b>この分岐はここまで</b><span>同じ前半から始まって、もう一方はどうなったかを見比べられます。</span><div className="branch-actions"><button type="button" onClick={()=>changeMode(mode==='pressure'?'open':'pressure')}>{mode==='pressure'?'非圧力側と見比べる':'圧力側と見比べる'}</button><button type="button" onClick={()=>changeMode('ambiguous')}>分岐点に戻る</button></div></div>}</div>{awaitingChoice&&pendingStage&&<div className="verify-choices"><b>{pendingStage.prompt}</b>{pendingStage.options.map(option=><button key={option.id} type="button" onClick={()=>choose(option)}>{option.text}</button>)}<small>{VERIFY_DISCLAIMER}</small></div>}{result&&<div className="verify-result"><b>{result.label}</b><p>{result.body}</p><small>{VERIFY_DISCLAIMER}</small></div>}<div className="composer"><span className={`typing ${typing?'is-typing':''}`}>{typing||' '}</span><div className="composer-row"><button aria-label="Add reaction" type="button">＋</button><input aria-label="Simulated message composer" value={typing?(typing.split('… ')[1]??''):''} readOnly placeholder="メッセージを入力"/><button aria-label="Send simulated message" type="button">↑</button></div></div></div>
