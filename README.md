...
- `app/page.tsx` — 中央集権型の会話データ、モード切替、自動送出、観察パネル
- `app/data/` — 共通前半・分散型・検証モード・自己点検の各データと判定
- `app/self/` — 自己点検の画面
